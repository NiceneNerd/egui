...
impl<'l> StripLayout<'l> {
    ...
    pub(crate) fn add(
        ...
        if selected {
            let select_rect = max_rect.expand2(0.5 * self.ui.spacing().item_spacing);
            self.ui
                .painter()
                .rect_filled(select_rect, 0.0, self.ui.visuals().selection.bg_fill);
        } else if striped {
        ...
        let response = self
            .ui
            .allocate_rect(allocation_rect, sense.unwrap_or_else(|| Sense::hover()));
